...
On \refline{id3_prune} the effect of the pruning parameter becomes clear.  %TODO: discuss further and provide reference
...
Considering the algorithm form the perspective a specific class, one can be more specific about the accuracy of an algorithm \cite{kubat:review}. For this class, and example is \myterm{positively classified} when the algorithm decides it is an instance of the class, and \myterm{negatively classified} when it decides an example is not an instance of the class. This algorithm will make one of these two types of errors:
...
\section{Experiment: The effect of test example selection} %c4-025
...
For this experiment the examples from the UCI Connect-4 dataset was used \cite{icu:data}. The following selection strategies are compared:
\begin{enumerate}
	\item $\myset W$: Select test examples from Win class only
	\item $\myset L$: Select test examples from Loss class only
	\item $\myset D$: Select test examples from Draw class only
	\item $\myset M$: Select mixed test examples
	\item $\myset B$: Select mixed test examples balanced to have the same ratio as the input set % proposed by fisher-1989-comparison or shavlik-1991-comparison (must be verified)
	\item $\myset F$: Use the entire set of examples as the test set
\end{enumerate} 
...
An element in the test set is not removed from the example set given to the classification algorithm.  This is an additional control that ensures that we are observing the measurement strategy; not the influence of having different examples presented to the classification algorithm.  Thus, the ID3 algorithm is applied once on the input, and the resulting decision tree is measured for each selection strategy.

For each subset selection strategy an observation is made $100$ times.  For each observation a random selection of $1/4$ of the total examples is chosen as the test set.  Each selection holds to the rules of the strategy being measured. 

The mean value and variance of these observations are then compared.   If the mean value differs considerably, the hypothesis is proven.  Also, the variance of each strategy is an indication of how accurate one can expect an arbitrary measurement using that strategy would be.  

      
\subsection{Results}
\subsection{Conclusion}
\section{Demonstration: A tic-tac-toe decision tree}
