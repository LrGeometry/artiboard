...
On \refline{id3_prune} the effect of the pruning parameter becomes clear.  
%TODO: discuss further and provide reference (the previous section mentiones pruning too)
...
\begin{enumerate}
	...
	\item $\mathcal B$: Select mixed test examples balanced to have the same ratio as the input set % proposed by fisher-1989-comparison or shavlik-1991-comparison (must be verified)
\end{enumerate} 
...
It is clear that the selection strategy has an effect on the measurement.  However, it could be that the differences has a lot to do with the unevenness of the examples in the dataset. Under these circumstances, the results suggests that $\mathcal B$ is a more precise measurement strategy.   
