...
\mydefs{Minimal Object Pruning} {A method to create a more generalised decision tree by not exploring nodes with too few examples are not explored. If a node has less than the provided cut-off value, further classification is not performed for that node.}

It is implemented on \refline{id3_prune}. If no pruning is needed, the algorithm must be provided with a cut-off value of zero.
...
\mydefc{Balanced Accuracy} {of a \myterm{Decision Tree}}{The \myterm{Decision Accuracy} of a tree such that the ratios between the classes in the  \myterm{test set} are equal to the ratios in the training set. }  

\mydefc{Trained Accuracy} {of a \myterm{Decision Tree}}{The \myterm{Decision Accuracy} where  \myterm{test set} is the entire training set.}  
...
The classification of a position is determined from the game lines  generated during the preceding stage of the macro cycle (see \refsec{learning-framework}). The positions in each game line are credited for their contribution (or their lack of contribution) to the outcome of the game. For example, if the outcome of the game is a win for the first player, positions chosen by the first player is ascribed with a higher likelihood to lead to a win than the positions chosen by the second player.  Thus, a tally of the outcomes of the game-lines in which it appears determines the position's classification. 

% the handling equal values
...
\subsection{Location regions}
% forces influencing the attribute choice
The feature language $\mathcal F$, described in \refsec{knowledge-language}, is adequate to {\sl express} an attribute, but it does not {\sl define} the set of attributes. An attribute used to encode a position consists of an  $\mathcal F$-expression that is interpreted as a boolean expression. 
...
It is also possible to select attributes based on game knowledge, but such an approach would spurn the zero knowledge principle introduced in \refsec{knowledge-granularity}.  

% defining the  regions and the attributes
For the current research, the attributes are created from easily identifiable regions on the board. 

\mydefs{Location Region Set} {A set of regions such that each region contains one and only one board location; and each location is in one and only one region}

The \myterm{Location Region Set} is both intuitive and simple.  A more sophisticated region set considers simple patterns.

\mydefs{Pattern Region Set} {A set of regions identified on the playable board. One region is the entire board, the other regions are the set of ranks (rows), the set of files (columns) and the set of diagonal lines. Also included is the set of concentric squares starting from the four squares in the centre of the board and ending with the squares on the border are also included as regions.}  

Selecting regions in this way is arbitrary enough to be considered free of game knowledge, and organised enough to be quickly recognisable by observation.  The  regions for the \myterm{Pattern Region Set} for the $8 \times 8$ board are listed in \reftab{attribute_regions} (see \refsec{language-symbol} for the notation used for squares).
\begin{table*}[h!]
\small
\centering
	\begin{tabular} {|l|l|l|}
...
\subsection{Region encodings}
\label{sec:pos_attribs}
% feature expressions as attributes
Recall from \refsec{language-features} that the \myterm{Fuzzy Occupation Feature} takes the form $\myset T \otimes \myset R$, where $\myset T$ is a subset of the set of occupation states, $\mypieces$. There is a very large number of expressions that is of this form; and it is from a subset called the Attribute Expression Set, $\myset E$, that the attributes are defined.  One attribute is declared for every $\mathcal F$-expression in $\myset E$:
\mydefc{Attribute Expression Set} {
...
Note that the size of the \myterm{attribute expression set} is:
\begin{equation}
|\myset E| = |\myset R| \times |\myset \mypieces|
\end{equation} 

\mydefc{Region encoding}{of a position, $p$}{A mapping from each element in an \myterm{Attribute Expression Set} to the values they take for $p$: $\myset R \mapsto \{Y,N\}$}

\mydefs{Location region encoding} {A \myterm{Region encoding} that employs \newline the \myterm{Location Region Set}}

\mydefs{Pattern region encoding} {A \myterm{Region encoding} that employs \newline the \myterm{Pattern Region Set}}

\subsection{Location encoding}
The \myterm{Location Region Set} has the advantage that each region has one and only one possible value (from $\mypieces $).  An as such a simpler encoding can devised when this set is used.

\mydefc{Location encoding}{of a position, $p$}{A mapping from each element in the \myterm{Location Region Set} to the value that elements has for $p$: $\myset R \mapsto \mypieces.$}
...
For each test selection strategy an observation is made $100$ times.  For each observation a random selection, $\myset S$ such that $|\myset S| = 1000$, are chosen to measure the \myterm{Decision Accuracy} of $T_t$. The elements in $\myset S$ are chosen to conform to the invariant implied by the strategy.  Fixing this number eliminates the situation where a larger test set produces more accurate measurements.
...
\subsection{Results}
From \reffig{c4-026-a} is is clear that accuracy decreases as the cut-off value increases.  The expected inverse relationship between size and cut-off is clearly visible  on \reffig{c4-026-s}.  A significant gain in size is notable between $32$ and $64$. 
\begin{figure} 
\center
...
% c4-027
\subsection{Objective}
Investigate the influence of \myterm{Minimal Object Pruning} on the \myterm{Trained Accuracy} of the classification tree. A higher cut-off is expected to produce a smaller tree but with less accuracy.   
\subsection{Method}
This experiment uses the \myterm{UCI Connect-Four data set}. $30$ measurements are taken, each incrementing the cut-off value by $8$, starting with a value of zero.
\subsection{Results}
The results are shown in \reffig{c4-027}.
\begin{figure} 
\centering
\subfigure % [Accuracy Measurements] 
{
	\includegraphics[width=0.45\textwidth]{artifacts/c4-027acc.eps}
}
\qquad
\subfigure %[Size Measurements] 
{
...
\section{Experiment: Influence of encoding on accuracy}
%c4-028
\subsection{Objective}
The difference on encoding schemes may influence the decision tree in size or in accuracy.
\subsection{Method}
Using the \myterm{UCI Connect-Four data set} we compare four different encodings:
\begin{enumerate}
	\item [SLE] Standard \myterm{Location Encoding}.  $6 \times 7 = 42$ attributes and $|\mypieces| = 3$ 
	\item [ELE] Enhanced \myterm{Location Encoding}. Empty space are marked as reachable or not. $6 \times 7 = 42$ attributes and $|\mypieces| = 4$  
	\item [SRE] Standard \myterm{Location Region Encoding} $6 \times 7 \times 3 = 126$ attributes and $|\mypieces| = 2$
	\item [ERE] Enhanced \myterm{Location Region Encoding} $6 \times 7 \times 4 = 168$ attributes and $|\mypieces| = 2$
\end{enumerate}
Each encoding is classified for different cut-off values starting from 0, incrementing by 16. We used the \myterm{Trained Accuracy} as measurement.

\subsection{Results}
The results are shown in \reffig{c4-028}.  From here it can be seem that regions (ERE and SRE) does a bit better on size and accuracy than the standard encodings. Also the enhanced encodings does a bit better.  
\begin{figure} 
\centering
\subfigure % [Accuracy Measurements] 
{
